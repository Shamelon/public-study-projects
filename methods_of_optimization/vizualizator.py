...
def optuna_optimize():
    ...
    def objective_gd(trial):
        ...
        step_entry.insert(0, trial.suggest_float("step", lb, rb))
        gradient_descent()
        if abs(values[-1]) < float(epsilon_entry.get()):
            return len(points)
        else:
    ...
    if study_object == "функция":
        ...
        study.optimize(objective_gd, n_trials=trials)
        step_entry.delete(0, tk.END)
        step_entry.insert(0, np.array(list(study.best_params.values()))[0])
        gradient_descent()
...
def gradient_descent():
    ...
    try:
        ...
        messagebox.showerror('Value Error', "Invalid step")
        return
...
optuna_entries = [left_border_entry, right_border_entry, trials_entry, object_entry]
