...
public class ExpressionParser implements TripleParser {
    ...
    private static class TripleExpressionParser extends BaseParser {
        ...
        private final char[] possibleStartsOfUnaryOperations = new char[] {'-', 'c'};
        ...
        boolean existBinOp = false; // Повстречали ли мы уже бинарную операцию

        String prevBinOp; // Предыдущая бинарная операция
        ...
        */
        public MyExpression parseExpression(boolean isBktExpression /*Выражение записано в скобках или нет*/) {
            ...
            while (true) {
                ...
                if (eof()) {
                    if (!isBinOpNext) {
                        throw error("Expression after " + prevBinOp + " expected");
                    }
                    if (isBktExpression) {
                        throw error("Unclosed bracket expression");
                    } else {
                        break;
                    }
                ...
                // Проверка, что следующий блок - бинарная операция
                    ...
                    if (isBinOpNext) {
                        ...
                        if (charIn(possibleStartsOfUnaryOperations)) {
                            expressions.add(parseUnaryOperation(take()));
                        } else if (existBinOp) {
                            throw error("Expression between \"" + prevBinOp + "\" and \"" + parseBinaryOperation() + "\" expected");
                        } else {
                            throw error("Expression before \"" + parseBinaryOperation() + "\" expected");
                        }
                    }
                // Проверка, что следующий блок - унарная операция
                    ...
                    if (!isBinOpNext) {
                        ...
                        throw error("Expect start of Binary Operation, given start of Unary Operation \"" + take() + "\"");
                    }
                // Проверка, что следующий блок - элемент
                    ...
                    if (!isBinOpNext) {
                        ...
                        if (between('0', '9')) {
                            throw error("Unexpected space between expressions");
                        } else {
                            parseElement(false);
                        }
                    }
        ...
        private BinOperation parseBinaryOperation() {
            ...
            if (take('+')) {
                ...
                expect("et");
                if (between('0', '9') || charIn(possibleStartsOfVariables))  {
                    throw error("Expected whitespace or start of Unary operation after \"set\", given \"" + take() + "\"");
                } else {
                    prevBinOp = "set";
                ...
                expect("lear");
                if (between('0', '9') || charIn(possibleStartsOfVariables))  {
                    throw error("Expected whitespace or start of Unary operation after \"clear\", given \"" + take() + "\"");
                } else {
                    prevBinOp = "clear";
                    return new BinOperation("clear", 11);
                }
            } else {
                throw error("Unknown start of binary operation \"" + take() + "\"");
            }
        }
        /*
        Unary Operation
        ...
        private MyExpression parseUnaryOperation(char opStart) {
            ...
            if (opStart == '-') {
                ...
                expect("ount");
                if (between('0', '9') || charIn(possibleStartsOfVariables)) {
                    throw error("Unknown character after \"count\" (\"" + take() + "\")");
                }
            } else {
                throw error("Unknown start of Unary Operation \"" + opStart + "\"");
            }
            skipWhitespace();
            if (take('(')) {
                ...
                return new UnOperation(op).convert(parseUnaryOperation('c'));
                ...
                throw error("Argument of Unary Operation \"" + op + "\" expected");
            }
        }
        ...
        */
        private MyExpression parseElement(boolean negative) {
            ...
            if (charIn(possibleStartsOfVariables)) {
                ...
                try {
                    ...
                    if (sb.charAt(0) == '-') {
                        throw error("Const \"" + sb + "\" < Integer.MIN_VALUE");
                    } else {
                        throw error("Const \"" + sb + "\" > Integer.MAX_VALUE");
                    }
                }
            } else {
        ...
        private static class UnOperation {
            ...
            public MyExpression convert(MyExpression exp1) {
                return switch (name) {
                    case "-" -> new CheckedNegate(exp1);
                    case "count" -> new CheckedCount(exp1);
                    default -> null;
                };
            }
        }
        ...
        private static class BinOperation {
            ...
            public MyExpression convert(MyExpression exp1, MyExpression exp2) {
                return switch (name) {
                    ...
                    case "set" -> new CheckedSet(exp1, exp2);
                    case "clear" -> new CheckedClear(exp1, exp2);
                    default -> null;
                };
            }
            @Override
